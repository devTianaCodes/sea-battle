function getCellClasses(cell, isInteractive) {
  if (cell.isHit && cell.isSunkReveal) {
    return "border-coral/70 bg-gradient-to-br from-coral/80 to-orange-400/70 shadow-[0_0_25px_rgba(255,107,107,0.35)]";
  }

  if (cell.isHit) {
    return "border-coral/70 bg-gradient-to-br from-coral/80 to-pink-400/70 shadow-[0_0_24px_rgba(255,107,107,0.25)]";
  }

  if (cell.isMiss) {
    return "border-cyan/25 bg-[radial-gradient(circle_at_center,rgba(0,212,255,0.22),rgba(0,212,255,0.02)_48%,transparent_64%)]";
  }

  if (cell.preview === "valid") {
    return "border-cyan/60 bg-cyan/20";
  }
  ...
  if (cell.shipId) {
    return "border-white/20 bg-gradient-to-br from-white/14 to-white/8";
  }

  return isInteractive
    ? "border-white/10 bg-white/5 hover:border-cyan/40 hover:bg-cyan/10"
