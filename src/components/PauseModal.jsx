...
export default function PauseModal({ open, onResume, onOpenInstructions, onMainMenu }) {
  ...
  return (
    <div className="fixed inset-[15px] z-[57] flex items-center justify-center rounded-[20px] bg-[#020817]/80 p-2 backdrop-blur-md animate-fade-in sm:p-3">
      <div className="glass-frosted flex max-h-full w-full max-w-2xl flex-col overflow-y-auto rounded-[1.35rem] p-3 sm:rounded-[2rem] sm:p-8">
        <p className="text-[0.62rem] uppercase tracking-[0.18em] text-cyan/70 sm:text-xs sm:tracking-[0.28em]">Paused</p>
        <h2 className="mt-2 font-display text-[1.6rem] text-foam sm:mt-3 sm:text-4xl">Battle on hold</h2>
        <p className="mt-2 max-w-xl text-[0.82rem] leading-6 text-slate-300 sm:mt-3 sm:text-sm sm:leading-7">
        ...
        <div className="mt-5 flex flex-wrap gap-2 sm:mt-8 sm:gap-3">
          <IconButton onClick={onResume} tone="success" className="px-3 py-2 text-[0.72rem] tracking-[0.08em] sm:text-sm">
            Resume
          </IconButton>
