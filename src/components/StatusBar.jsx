export default function StatusBar({
  ...
  return (
    <header
      ...
      className="status-bar glass-panel rounded-[1.1rem] border-b border-cyan/20 px-2.5 py-2 sm:rounded-[1.25rem] sm:px-4 sm:py-2.5"
    >
      <div className="status-top-row flex items-center justify-between gap-3">
        <div className="text-[0.74rem] font-semibold uppercase tracking-[0.18em] text-cyan-100 sm:text-[0.82rem] sm:tracking-[0.22em]">
          Sea Battle
        </div>

        <div className={`status-text status-turn ${isPlayerTurn ? "is-active-turn" : "text-slate-300"}`}>
      ...
      <p
        className="mt-1 max-w-3xl break-words text-[0.8rem] leading-[1.2rem] text-slate-300 sm:mt-1.5 sm:text-[0.88rem] sm:leading-5"
        role="status"
        aria-live="polite"
      >
        {announcement}
